...
mod tests {
    ...
    pub fn test_make_instruction() {
        let (mut svm, payer, mint_a, mint_b, escrow, vault, maker_ata_a, amount_to_receive, amount_to_give, bump) =
            make_escrow_setup();
        ...
        assert_eq!(maker_state.amount, 1_000_000_000 - amount_to_give);
        ...
        let _ = (mint_a, mint_b, program_id);
    }

    #[test]
    pub fn test_take_instruction() {
        let (mut svm, maker, mint_a, mint_b, escrow, vault, maker_ata_a, amount_to_receive, amount_to_give, _) =
            make_escrow_setup();
        ...
        assert!(svm.get_account(&maker_ata_a).is_some());
    }

    #[test]
    pub fn test_cancel_instruction() {
        let (mut svm, maker, mint_a, _, escrow, vault, maker_ata_a, _, amount_to_give, _) =
            make_escrow_setup();
    ...
    #[test]
    pub fn test_take_fails_for_underfunded_taker() {
        let (mut svm, maker, mint_a, mint_b, escrow, vault, maker_ata_a, amount_to_receive, amount_to_give, _) =
            make_escrow_setup();
        ...
        let taker_ata_b = CreateAssociatedTokenAccount::new(&mut svm, &taker, &mint_b)
            ...
            .send()
            .unwrap();
        MintTo::new(&mut svm, &maker, &mint_b, &taker_ata_b, amount_to_receive - 1)
            .send()
            .unwrap();
        ...
        let tx = Transaction::new(&[&taker], message, svm.latest_blockhash());
        let result = svm.send_transaction(tx);
        assert!(result.is_err(), "taker must have enough B to complete the trade");
