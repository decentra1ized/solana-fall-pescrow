...
mod tests {
    ...
    fn setup() -> (LiteSVM, Keypair) {
        ...
        let program_data = std::fs::read(&so_path).unwrap_or_else(|e| panic!("Failed to read program SO file at {}: {e}. Run `cargo build-sbf` first.", so_path.display()));
    ...
    pub fn test_take_instruction() {
        ...
        let taker_ata_b = CreateAssociatedTokenAccount::new(&mut ctx.svm, &taker, &ctx.mint_b).owner(&taker.pubkey()).send().unwrap();
        MintTo::new(&mut ctx.svm, &taker, &ctx.mint_b, &taker_ata_b, ctx.amount_to_receive).send().unwrap();

        let taker_ata_a = spl_associated_token_account::get_associated_token_address(&taker.pubkey(), &ctx.mint_a);
        let maker_ata_b = spl_associated_token_account::get_associated_token_address(&ctx.maker.pubkey(), &ctx.mint_b);

        let associated_token_program = ASSOCIATED_TOKEN_PROGRAM_ID.parse::<Pubkey>().unwrap();
        
        let maker_sol_before = ctx.svm.get_account(&ctx.maker.pubkey()).map(|a| a.lamports).unwrap_or(0);
        ...
        let res = ctx.svm.send_transaction(transaction).unwrap();
        println!("Take CUs Consumed: {}", res.compute_units_consumed);

        let taker_ata_a_acc = ctx.svm.get_account(&taker_ata_a).unwrap();
        ...
        let maker_ata_b_acc = ctx.svm.get_account(&maker_ata_b).unwrap();
        ...
        let vault_acc = ctx.svm.get_account(&ctx.vault);
        assert!(vault_acc.is_none() || vault_acc.unwrap().lamports == 0);

        let escrow_acc = ctx.svm.get_account(&ctx.escrow);
        assert!(escrow_acc.is_none() || escrow_acc.unwrap().lamports == 0);
        
        let maker_sol_after = ctx.svm.get_account(&ctx.maker.pubkey()).map(|a| a.lamports).unwrap_or(0);
    ...
    pub fn test_cancel_instruction() {
        ...
        assert_eq!(maker_ata_a_state.amount, 1000000000); // 1000 original
    ...
    pub fn test_take_insufficient_funds() {
        ...
        MintTo::new(&mut ctx.svm, &taker, &ctx.mint_b, &taker_ata_b, ctx.amount_to_receive - 1).send().unwrap();
        ...
        assert!(result.is_err(), "Taker with insufficient funds should fail");
    }
    ...
    pub fn test_cancel_stranger() {
        ...
        let cancel_ix = Instruction {
            program_id: program_id(),
            accounts: vec![
                AccountMeta::new(stranger.pubkey(), true), // Stranger trying to cancel
                AccountMeta::new(ctx.mint_a, false),
                AccountMeta::new(ctx.escrow, false),
                AccountMeta::new(ctx.vault, false),
                AccountMeta::new(ctx.maker_ata_a, false),
