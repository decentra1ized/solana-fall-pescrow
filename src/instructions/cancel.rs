...
// Account order:
// 0: maker, signer and the canonical escrow owner
// 1: mint_a, token mint hidden in the escrow record
// 2: escrow_account, PDA owned by the program, closed at the end
// 3: vault, ATA for escrow PDA / mint A, closed at the end
// 4: maker_ata_a, destination ATA for A owned by maker
// 5: token_program
...
pub fn process_cancel_instruction(accounts: &mut [AccountView]) -> ProgramResult {
    ...
    let expected_escrow = pinocchio::Address::from(pinocchio_pubkey::derive_address(
